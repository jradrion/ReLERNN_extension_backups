...
def mapOverlap(wins, last_win, mask):
    """
    return a three-element list with the first element being the proportion overlap of each recombination rate window,
    the second element being the rate for each window,
    and the third being the last window before breaking to expidite the next loop
    """
    chrom = wins[0].split(":")[0]
    a = wins[1]
    ...
    try:
        for i in range(last_win, len(mask[chrom])):
            ...
            if b < x:
                ...
                if a >= x and b <= y:
                    return [[1.0], [mask[chrom][i][2]], i]
                elif a >= x and b > y:
                    win_prop = (y-a)/float(b-a)
                    prop[0].append(win_prop)
                    prop[1].append(mask[chrom][i][2])
                    prop[2] = i
                elif b <= y and a < x:
                    win_prop = (b-x)/float(b-a)
                    prop[0].append(win_prop)
